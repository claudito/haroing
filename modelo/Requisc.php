...
function lista($tipo)
{
	...
	$modelo    = new Conexion();
	$conexion  = $modelo->get_conexion();
	$query     = "SELECT r.id, r.numero, concat(u.nombres ,' ', u.apellidos) as usuario, r.fecha_inicio, r.fecha_fin, r.comentario,c.descripcion as centro_costo, r.ot as orden_trabajo, a.descripcion as area, r.tipo, r.estado, r.prioridad
from requisc as r
left join area as a on r.area = a.id
left join usuario as u on r.id_usuario = u.id
left join centro_costo as c on r.centro_costo = c.id WHERE r.tipo=:tipo";
	$statement = $conexion->prepare($query);
...
public function consulta($id,$campo,$tipo)
{
    ...
    $modelo    = new Conexion();
    $conexion  = $modelo->get_conexion();
    $query     = "SELECT r.id, r.numero, concat(u.nombres ,' ', u.apellidos) as usuario, r.fecha_inicio, r.fecha_fin, r.comentario,c.id as idcentro_costo,c.descripcion as centro_costo, r.ot as orden_trabajo,a.id as idarea,a.descripcion as area, r.tipo, r.estado, r.prioridad
from requisc as r
left join area as a on r.area = a.id
left join usuario as u on r.id_usuario = u.id
left join centro_costo as c on r.centro_costo = c.id WHERE r.numero=:id AND r.tipo=:tipo";
    $statement = $conexion->prepare($query);
    $statement->bindParam(':id',$id);
    $statement->bindParam(':tipo',$tipo);
    $statement->execute();
