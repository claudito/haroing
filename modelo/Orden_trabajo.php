...
function lista()
{
	...
	$modelo    = new Conexion();
	$conexion  = $modelo->get_conexion();
	$query     = "SELECT of.id, of.codigo, c.id as idcliente, c.contacto as cliente, u.id as idunidad, u.descripcion as unidad, of.descripcion, of.fecha_inicio, of.fecha_fin, of.cantidad, of.observacion, of.estado
from ord_fab as of
left join cliente as c on of.codigo_cliente = c.id
left join unidad as u on of.unidad = u.codigo";
	$statement = $conexion->prepare($query); 
	$statement->execute();
	$result = $statement->fetchAll();
	return $result;
...
from ord_fab as of
left join cliente as c on of.codigo_cliente = c.id
left join unidad as u on of.unidad = u.codigo WHERE id=:id";
    $statement = $conexion->prepare($query);
    $statement->bindParam(':id',$id);
    $statement->execute();
